...
def print_spicy_foods(spicy_foods)
  spicy_foods.each do |food|
    foods_heat_levels = '🌶 ' * food[:heat_level]
    spicy_food_heat_level = "#{food[:name]} (#{food[:cuisine]}) | Heat Level: #{foods_heat_levels}"
  puts spicy_food_heat_level
  end 
end
...
cuisine = "Thai"
puts get_spicy_food_by_cuisine(spicy_foods, cuisine)
...
def print_spiciest_foods(spicy_foods)
    spicy_foods.each do |food|
      if food[:heat_level] > 5
        spicy_foods_heat_level = '🌶' * food[:heat_level]
        puts "#{food[:name]} (#{food[:cuisine]}) | Heat Level: #{spicy_foods_heat_level}"
      end
    end
  end
